...
class CSVOutputFormat(KVWriter):
    ...
    def writekvs(self, kvs):
        ...
        kvs = {str(k).replace(",", ">>"): v for (k, v) in kvs.items()}
...
class TensorBoardOutputFormat(KVWriter):
    ...
    def write_video(self, k, v):
        ...
        w.add_video(
            f"video/{name}",
            torch.tensor(v).permute(0, 3, 1, 2)[None, :],
            self.step,
            fps=10,
        )
...
def logkvs_mean_std(d, prefix=None):
    """
    Log a dictionary of key-value pairs with averaging over multiple calls
    """
    for k, v in d.items():
...
<source src="FILENAME" type="video/TYPE">
        ...
        write_html(
            html.replace("FILENAME", f).replace("TYPE", f.split(".")[-1]), f"{f}.html"
        )
    return ani
...
def write_html(txt, f):
    ...
    if run is not None:
        ...
        url = (
            run.get_url()
            .replace("wandb.ai", "api.wandb.ai/files")
            .replace("/runs/", "/")
        )
...
def configure(
    ...
    format_strs: Union[List[str], str, None] = None,
    date=False,
    **kwargs,
):
    ...
    if log_suffix is not None:
        output_formats = [
            make_output_format(f, dir, log_suffix, **kwargs) for f in format_strs
        ]
