...
def infer_end():
    ...
    agent.load_model(dir, id=9500)
    my_actions = []
    rewards = []
    q_values = []
    env.reset()
    end_indices = np.where(terminals == 1)[0]
    previous = end_indices - 1
    end_indices_tot = np.concatenate((end_indices, previous), axis=0)
    for i in end_indices_tot:
        ...
        torch_action = torch.FloatTensor(agent_action.reshape(1, -1)).to(agent.device)
        # q_value = agent.critic.q1(torch_state, torch_action, t).cpu().detach().numpy()
        q_value = agent.critic.q1(torch_state, torch_action).cpu().detach().numpy()
        my_actions.append(agent_action)
        # rewards.append(q_value.item())
