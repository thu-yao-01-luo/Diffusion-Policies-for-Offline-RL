...
class Diffusion_AC(object):
    def __init__(self, state_dim, action_dim, max_action, device, args: Config):
        merged_state_dim = state_dim * args.state_len
        merged_action_dim = action_dim * args.action_len
        self.state_len = args.state_len
        self.action_len = args.action_len
        self.eval_steps = args.eval_steps
        self.model = MLP(state_dim=merged_state_dim, action_dim=merged_action_dim, device=device)
        self.actor = Diffusion(state_dim=merged_state_dim, action_dim=merged_action_dim, model=self.model, max_action=max_action,
        ...
        self.critic = TestCritic(merged_state_dim, merged_action_dim, max_time_step=args.T).to(device)
        ...
        self.merged_action_dim = merged_action_dim
        self.discount = args.discount ** args.eval_steps    
        self.discount2 = args.discount2 ** args.eval_steps
        self.tau = args.tau
        self.eta = args.eta
        self.device = device
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None):
        ...
        for _ in range(iterations):
            ...
            t = torch.randint(self.len_rollout - 1, self.actor.n_timesteps, (batch_size,), device=self.device).long() # (b,)
            q_t = t + 1
            q_prev_t = q_t - self.len_rollout
            noisy_action = self.actor.q_sample(action, t, noise)
            with torch.no_grad():
                tmp = noisy_action
                for i in range(self.len_rollout):
                    tmp = self.ema_model.p_sample(tmp, t - i, state)
                denoised_noisy_action_ema = tmp
                if np.random.uniform() > 0.5:
                    target_v = self.critic_target.q1(state, denoised_noisy_action_ema, q_prev_t) # (b, 1)->(b,)
                else:
                    target_v = self.critic_target.q2(state, denoised_noisy_action_ema, q_prev_t)
            ...
            if self.step % self.policy_freq == 0:
                tmp = noisy_action 
                for i in range(self.len_rollout):
                    tmp = self.actor.p_sample(tmp, t - i, state)
                denoised_noisy_action = tmp
                q1, q2 = self.critic.q(state, denoised_noisy_action, q_prev_t) # this is the bug!
                q_value = q1
                if np.random.uniform() > 0.5:
    ...
    def sample_action(self, state, noise_scale=0.0):
        ...
        if len(self.sample_states) < self.state_len:
            return None
        target_state = torch.cat(self.sample_states[-self.state_len:], dim=1) # (b, o*state_len)
        self.sample_states.pop(0)
        if self.resample:
            state_rpt = torch.repeat_interleave(target_state, repeats=50, dim=0) # (50b, o)
            with torch.no_grad():
            ...
            act_vector = action[idx].cpu().data.numpy().squeeze()
            return [act_vector[i * self.action_dim: (i + 1) * self.action_dim] for i in range(self.eval_steps)]
        else:
            action = self.actor.sample(target_state)
            action += noise_scale * torch.randn_like(action)
            action = action.clamp(-self.max_action, self.max_action)
            # return action.cpu().data.numpy().flatten()
            # return action.cpu().data.numpy().squeeze()
            act_vector = action.cpu().data.numpy().squeeze()
            return [act_vector[i * self.action_dim: (i + 1) * self.action_dim] for i in range(self.eval_steps)]
