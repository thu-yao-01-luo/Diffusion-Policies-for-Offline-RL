...
class Diffusion_DD(object):
    def __init__(self,
        ...
        self.behavior_model = MLP(state_dim=state_dim,
                          action_dim=action_dim, device=device)
        self.behavior_actor = Diffusion(state_dim=state_dim, action_dim=action_dim, model=self.behavior_model, max_action=max_action,
                               beta_schedule=beta_schedule, n_timesteps=n_timesteps, scale=scale, predict_epsilon=predict_epsilon).to(device)
        self.behavior_actor_optimizer = torch.optim.Adam(self.behavior_actor.parameters(), lr=lr)
    ...
    def fit_dataset(self, dataset, batch_size=100, epochs=10000):
        for epoch in range(epochs):
            state, action, next_state, reward, not_done = dataset.sample(
                batch_size)
            loss = self.behavior_actor.loss(action, state)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None):
        ...
        for _ in range(iterations):
            ...
            bc_loss = self.behavior_actor.p_losses(new_action, state, t)
            bc_loss = bc_loss if not self.debug \
            else self.behavior_actor.loss_to_verify(action, state)
            if self.actor.predict_epsilon:
                self.actor.predict_epsilon = False
                true_bc_loss = self.behavior_actor.p_losses(new_action, state, t)
